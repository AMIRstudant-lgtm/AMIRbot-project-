...
async def handle_text(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # كشف طلب صورة - يجب أن تبدأ الجملة بالكلمة
    text_lower = text.lower().strip()
    img_triggers = ["صورة ","صوره ","ارسم ","generate image","image of","draw ","/image"]
    is_img = (any(text_lower.startswith(kw.strip()) for kw in img_triggers)
              or text_lower in ["صورة","صوره"])
